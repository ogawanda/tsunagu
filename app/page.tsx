...
export default function Home() {
  ...
  const [selectedShift, setSelectedShift] = useState("すべて");
  ...
  const filtered = handovers.filter((h) => {
    ...
    if (selectedShift !== "すべて" && (h.shift ?? "朝") !== selectedShift) return false;
  ...
  const shiftGroups = SHIFTS.map((shift) => ({
    shift,
    items: filtered.filter((h) => (h.shift ?? "朝") === shift),
  ...
  const shiftColor = (s: string) => {
    if (s === "朝") return "bg-orange-50 text-orange-600 border-orange-200";
    if (s === "昼") return "bg-sky-50 text-sky-600 border-sky-200";
    return "bg-indigo-50 text-indigo-600 border-indigo-200";
  };

  const renderCard = (item: Handover) => (
    <div
      key={item.id}
      ...
      <div className="p-4">
            ...
            {item.image_url && (
              <div className="mt-2">
                <img
                  src={item.image_url}
                  alt="添付画像"
                  className="max-w-full rounded-lg border border-slate-200 max-h-48 object-cover cursor-pointer"
        ...
        {editingId !== item.id && (
          <div className="mt-2 flex gap-2 flex-wrap">
            ...
            <button
              onClick={() => duplicateHandover(item)}
              className="text-xs text-slate-500 bg-slate-100 hover:bg-green-50 hover:text-green-600 px-3 py-1 rounded-lg transition-colors"
            >
              複製
            </button>
            <button
            ...
            {shiftGroups.map(({ shift, items }) => (
              <div key={shift}>
                <div className={`flex items-center gap-2 mb-3 px-1`}>
                  <span className={`text-xs font-bold px-3 py-1 rounded-full border ${shiftColor(shift)}`}>
                    {shift}シフト
                  </span>
